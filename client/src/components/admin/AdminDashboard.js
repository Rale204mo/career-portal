...
const AdminDashboard = () => {
  ...
  const handlePublishAdmissions = async (institutionId) => {
    try {
      const result = await realApi.publishAdmissions(institutionId, {
      ...
      <Modal show={showPublishAdmissions} onHide={() => setShowPublishAdmissions(false)}>
        ...
        <Modal.Body>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Select>
              <option>Limkokwing University</option>
              <option>Botho University</option>
              <option>Gaborone University</option>
            </Form.Select>
          </Form.Group>
          <Alert variant="info">
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowPublishAdmissions(false)}>
            Cancel
          </Button>
          <Button variant="success" onClick={() => handlePublishAdmissions('limkokwing')}>
            Publish Admissions
          </Button>
        </Modal.Footer>
      </Modal>
